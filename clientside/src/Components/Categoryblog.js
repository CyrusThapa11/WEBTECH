...
function Categoryblog() {
  ...
  function loader() {
    const toastId = toast.warning("Getting Blogs for You", {
      autoClose: false,
    });
    return toastId;
  }
  useEffect(() => {
    async function getResults() {
      ...
      try {
        const locid = loader();
        const res = await axios.post("http://localhost:4200/bycategory", cat);
        toast.dismiss(locid);
        // console.log(res);
          ...
          {newData?.length ? (
            ...
            <div className="text-center text-dark font-lg">
              <h1 className="display-5">
                404: No Blog found in this category!
              </h1>
              <a
                className="btn btn-info "
                role="button"
                href="http://localhost:3000/newblog"
              >
                {" "}
                Start Writing Now
              </a>
