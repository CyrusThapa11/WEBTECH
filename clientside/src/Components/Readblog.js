...
function Readblog() {
  ...
  useEffect(() => {
    async function getData() {
      ...
      if (
        res.data.details[0].usersliked.includes(
          JSON.parse(localStorage.getItem("whoisthis")).username
        )
      ) {
        setliked(1);
      }
  ...
  const updatelikepos = async () => {
    // console.log(val)
    const res = await axios.post("http://localhost:4200/updatelikepos", {
      id: blogid,
      // value: val,
      user: JSON.parse(localStorage.getItem("whoisthis")).username,
    });
    console.log(res.data.details);
    if (res.data.message === "-1") console.log("update failed");
  ...
  const updatelikeneg = async () => {
    // console.log(val)
    const res = await axios.post("http://localhost:4200/updatelikeneg", {
      id: blogid,
      user: JSON.parse(localStorage.getItem("whoisthis")).username,
    });
    console.log(res.data.details);
